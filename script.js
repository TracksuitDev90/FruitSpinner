/* ---------- Fruit config (flat colors) ---------- */
/* Blackberry uses a darker purple and a distinct emoji stand-in.
   Lime has no universal emoji; we use a green circle. */
const FRUITS = [
...
/* ---------- High-PPI canvas (supersampled) ---------- */
function resizeCanvas(){
  const oversample = 2;                                      // render > device pixel density
...
/* ---------- Labels: clean sans, no outline, gentle tracking ---------- */
function drawArcLabel(text, radius, centerAngle, arcAngle, maxHeight){
  ...
  while(fontPx>10){
    ...
    const spacing=fontPx*0.04; // tighter kerning
  ...
  for(const ch of label){
    ...
    ctx.fillStyle='#fff';                 // unified color
    ctx.fillText(ch, 0, 0);               // no stroke outline
    ctx.restore();
...
/* ---------- Draw wheel (flat two-tone; sharp separators) ---------- */
function draw(){
  ...
  for(let i=0;i<FRUITS.length;i++){
    ...
    if(f.split){
      ...
      // center split line (snap to pixel for crispness)
  ...
  // center hub only (no outer bezel)
...
/* ---------- Spin with weight + ticking pointer wobble ---------- */
...
function spin(){
  ...
  const extraTurns    = prefersReduced ? 0 : (5 + secureRandomInt(4));        // heavier spin
  const delta         = normalizeAngle(baseTarget - startRotation, extraTurns);
  const duration      = prefersReduced ? 900 : (5600 + secureRandomInt(1200)); // slightly longer

  const t0 = performance.now();
  state.spinning = true; spinBtn.disabled = true; statusEl.textContent = 'Spinning…';

  prevRot = startRotation;
  prevPhase = ((startRotation / sliceAngle) % 1 + 1) % 1;

  // pointer physics constants
  let lastFrame = t0;
  const K = 140;           // spring stiffness
  const DAMP = 16;         // damping
  const IMPULSE = 3.2;     // tick impulse magnitude

  function frame(now){
    const dt = Math.max(0, (now - lastFrame) / 1000); // seconds
    lastFrame = now;
    ...
    // angular velocity estimate for “weighty” feel & impulse scaling
    const vel = (current - prevRot) / Math.max(1e-6, (dt||1/60));

    // detect crossing of slice boundaries to trigger a tick
    const phase = ((current / sliceAngle) % 1 + 1) % 1; // 0..1 within a slice
    if (vel > 0 && phase < prevPhase || vel < 0 && phase > prevPhase){
      // apply an impulse to wobble, direction depends on spin
      state.wobbleVel += (vel > 0 ? 1 : -1) * IMPULSE * clamp(Math.abs(vel)*0.6, 0.4, 1.8);
    }
    prevPhase = phase;

    // damped spring for pointer wobble
    const wA = state.wobbleAngle * (Math.PI/180);
    let wV = state.wobbleVel;
    const acc = -K * wA - DAMP * wV;
    wV += acc * dt;
    const newWA = wA + wV * dt;
    state.wobbleVel = wV;
    state.wobbleAngle = clamp(newWA * (180/Math.PI), -18, 18); // limit
    ...
    if (t < 1){
      ...
      // final snap + slight residual wobble
...
function persistDisabled(){ const data=FRUITS.map(f=>f.split?{split:true,sub:f.subDisabled}:!!f.disabled); localStorage.setItem('simsFruitDisabled_v8', JSON.stringify(data)); }
function restoreDisabled(){
  const raw=localStorage.getItem('simsFruitDisabled_v8'); if(!raw) return;
  try{
...
/* ---------- CodePen bubbly-button JS (copied core) ---------- */
...
/* ---------- Randomize bubble colors/sizes (bigger) ---------- */
...
function randomGradients(n){
  const parts=[]; for(let i=0;i<n;i++){ const col=PALETTE[secureRandomInt(PALETTE.length)];
    const pct = 18 + secureRandomInt(18); // bigger circles
    parts.push(`radial-gradient(circle, ${col} ${pct}%, transparent ${pct}%)`);
  }
  return parts.join(', ');
}
function randomSizes(n, min=16, max=34){
...
function setRandomBubbleStyle(btn){
  btn.style.setProperty('--top-bg', randomGradients(12));
  btn.style.setProperty('--bot-bg', randomGradients(10));
  btn.style.setProperty('--top-size', randomSizes(12,16,36));
  btn.style.setProperty('--bot-size', randomSizes(10,16,36));
}
...
function resetAll(){ FRUITS.forEach(f=>{ if(f.split) f.subDisabled=[false,false]; else f.disabled=false; });
  localStorage.removeItem('simsFruitDisabled_v8'); state.rotation=0; state.spinning=false; spinBtn.disabled=false; state.wobbleAngle=0; state.wobbleVel=0; draw(); }

...
const FRUITS = [
  ...
  // Apple: visual split but counts as one pick
  { name: 'Apple', split: true, bases: ['#d62828', '#2aa74a'], emojis: ['🍎','🍏'], subDisabled:[false,false] },

  // Darker, richer Blackberry
  { name: 'Coconut',    base: '#7a4f2a', emoji: '🥥' },
  { name: 'Blackberry', base: '#2a0a59', emoji: '🫐' } // darker & richer
];
...
function desaturate(hex,amt=0.8,lift=0.05){ const {r,g,b}=hexToRgb(hex); const {h,s,l}=rgbToHsl(r,g,b); const {r:rr,g:rg,b:rb}=hslToRgb(h,clamp(s*(1-amt),0,1),clamp(l+lift,0,1)); return rgbToHex(rr,rg,rb); }
function pickLabelColor(hex){ const {r,g,b}=hexToRgb(hex); const lum=0.2126*(r/255)+0.7152*(g/255)+0.0722*(b/255); return lum>0.6 ? '#111' : '#fff'; }
...
const state = { dpr: Math.max(1, window.devicePixelRatio||1), sizeCSS:720, rotation:0, spinning:false, innerCoverage:0.88, lastFocus:null };
function readCSSVars(){ const v=getComputedStyle(document.documentElement).getPropertyValue('--inner-coverage'); const n=parseFloat(v||'0.88'); state.innerCoverage=Number.isFinite(n)?n:0.88; }
...
function drawArcLabel(text, radius, centerAngle, arcAngle, color, maxHeight){
  const label=text.toUpperCase(); const baseFontPx=Math.min(maxHeight,Math.floor(state.sizeCSS*0.052)); let fontPx=baseFontPx; const pad=Math.max(6,state.sizeCSS*0.012);
  ctx.textBaseline='middle'; ctx.textAlign='center';
  let totalAngle;
  while(fontPx>10){
    ctx.font=`900 ${fontPx}px Inter, system-ui, sans-serif`;
    const spacing=fontPx*0.06; let width=0; for(const ch of label) width+=ctx.measureText(ch).width+spacing; width-=spacing;
    totalAngle=(width+pad*2)/radius; if(totalAngle<=arcAngle*0.9) break; fontPx--;
  }
  const startAngle=centerAngle-totalAngle/2; const stroke=(color==='#fff')?'rgba(0,0,0,.55)':'rgba(255,255,255,.55)'; const spacing=fontPx*0.06;
  ctx.save(); ctx.rotate(startAngle);
  for(const ch of label){
    const w=ctx.measureText(ch).width; const charAngle=(w+spacing)/radius;
    ctx.save(); ctx.rotate(charAngle/2); ctx.translate(radius,0); ctx.rotate(Math.PI/2);
    ctx.lineWidth=Math.max(1,fontPx*0.16); ctx.strokeStyle=stroke; ctx.fillStyle=color; ctx.strokeText(ch,0,0); ctx.fillText(ch,0,0);
    ctx.restore(); ctx.rotate(charAngle);
  }
  ctx.restore();
}
...
function resizeCanvas(){
  ...
  canvas.width=Math.round(size*state.dpr); canvas.height=Math.round(size*state.dpr);
...
/* ---------- Draw wheel ---------- */
function draw(){
  const size=state.sizeCSS; const R=size/2*0.96; const innerR=R*state.innerCoverage; const ringT=R-innerR; const sliceAngle=TWO_PI/FRUITS.length;
  ctx.clearRect(0,0,size,size); ctx.save(); ctx.translate(size/2,size/2); ctx.rotate(state.rotation);
  ...
  // bezel + hub
  ctx.lineWidth=Math.max(2,size*0.008); ctx.strokeStyle='rgba(255,255,255,.18)';
...
function normalizeAngle(a, extra){ a%=TWO_PI; if(a>Math.PI)a-=TWO_PI; if(a<-Math.PI)a+=TWO_PI; return a + TWO_PI*extra; }
...
function spin(){
  ...
  function frame(now){
    ...
    if(t<1) requestAnimationFrame(frame);
      ...
      else { f.disabled=true; afterPick(f.name,[f.emoji],[f.base]); }
    }
  }
...
/* ---------- Modal & persistence ---------- */
function setAppInert(b){ [headerEl,mainEl].forEach(el=>{ if(!el)return; if(b){ el.setAttribute('inert',''); el.setAttribute('aria-hidden','true'); } else { el.removeAttribute('inert'); el.removeAttribute('aria-hidden'); } }); }
function openModal(title, emojis, colors){
  modalTitle.textContent=title;
  const base=colors[0]; const accent=colors[1] ? colors[1] : lighten(base,0.25);
  if(colors.length===2){ modalPanel.style.borderImage=`linear-gradient(90deg, ${colors[0]} 0 50%, ${colors[1]} 50% 100%) 1`; }
  else{ modalPanel.style.borderImage='unset'; modalPanel.style.borderColor=base; }
  modalPanel.style.background=`linear-gradient(180deg, ${accent}22, #121212 40%)`;
  modalIcons.textContent=''; const span=document.createElement('div'); span.textContent=emojis.join(' '); modalIcons.appendChild(span);
  state.lastFocus=document.activeElement; setAppInert(true); modal.hidden=false; modalOk.focus();
  function handler(e){ if(e.key==='Escape'){ closeModal(); } }
  modal._esc=handler; document.addEventListener('keydown', handler);
}
function closeModal(){
  modal.hidden=true; setAppInert(false);
...
function afterPick(label, emojis, colorList){
  ...
  const noneLeft = remaining===0; spinBtn.disabled=noneLeft;
  if(noneLeft){
...
function persistDisabled(){ const data=FRUITS.map(f=>f.split?{split:true,sub:f.subDisabled}:!!f.disabled); localStorage.setItem('simsFruitDisabled_v6', JSON.stringify(data)); }
function restoreDisabled(){ const raw=localStorage.getItem('simsFruitDisabled_v6'); if(!raw)return; try{ const data=JSON.parse(raw);
  data.forEach((v,i)=>{ const f=FRUITS[i]; if(!f)return; if(f.split && v && v.split) f.subDisabled=[!!v.sub?.[0], !!v.sub?.[1]]; else if(!f.split) f.disabled=!!v; }); }catch{}
}
...
/* ---------- Our additions: vary bubble colors & sizes each press ---------- */
...
function randomGradients(n){
  // Build a list of n radial-gradients with random colors/sizes
  const parts=[]; for(let i=0;i<n;i++){
    const col = PALETTE[secureRandomInt(PALETTE.length)];
    const stop = 12 + secureRandomInt(14); // 12–25%
    parts.push(`radial-gradient(circle, ${col} ${stop}%, transparent ${stop}%)`);
  }
  return parts.join(', ');
}
...
function setRandomBubbleStyle(btn){
  // Top uses 9 layers, Bottom uses 7 (matching CodePen counts)
...
/* Wire clicks to both: randomize + run CodePen animation + spin */
spinBtn.addEventListener('click', (e)=>{
  setRandomBubbleStyle(spinBtn);
  animateButton(e);  // CodePen effect
...
spinBtn.addEventListener('keydown', (e)=>{
  if(e.key==='Enter' || e.key===' '){
    setRandomBubbleStyle(spinBtn);
    animateButton({target:spinBtn, preventDefault(){}});
    if(!state.spinning) spin();
    e.preventDefault();
  }
});
...
function resetAll(){ FRUITS.forEach(f=>{ if(f.split) f.subDisabled=[false,false]; else f.disabled=false; });
  localStorage.removeItem('simsFruitDisabled_v6'); state.rotation=0; state.spinning=false; spinBtn.disabled=false; draw(); }

restoreDisabled(); readCSSVars(); resizeCanvas(); draw();